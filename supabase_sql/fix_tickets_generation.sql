-- ============================================================================
-- FIX TICKETS GENERATION (PERFECT VERSION)
-- ============================================================================
  ...
  IF NEW.status = 'approved' AND (OLD.status IS NULL OR OLD.status != 'approved') THEN
    ...
    LOOP
      ...
      FOR ticket_count IN 1..order_item.quantity LOOP
        -- Usamos gen_random_uuid() que es nativo de Postgres (no requiere extensión uuid-ossp)
...
BEGIN
  ...
  LOOP
    ...
    LOOP
      ...
      FOR ticket_count IN 1..order_item.quantity LOOP
        new_ticket_id := gen_random_uuid();
        INSERT INTO public.tickets (id, user_id, ticket_type, price, payment_status, payment_id_mp, qr_code_data, purchase_date, updated_at)
        VALUES (new_ticket_id, order_item.user_id, order_item.ticket_type_name, order_item.unit_price, 'approved', rec.mp_payment_id, new_ticket_id::text, now(), now());
      END LOOP;
    END LOOP;
  END LOOP;
END $$;
